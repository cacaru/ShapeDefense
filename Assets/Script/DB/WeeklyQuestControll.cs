...
/// </summary>
/// 
public class WeeklyQuestControll : QuestController
{
    [SerializeField] private GameObject W_Content;
    [SerializeField] private GameObject W_AllReciveBtn;
    ...
    public override void Show() {
        ...
        // list를 재정렬
        ArrayList ori_list = datahub.WeeklyQuest;
        int size = ori_list.Count;

        for (int i = 1; i < size; i++) {
            WeeklyQuest achieve = ori_list[i] as WeeklyQuest;
            // 완료된것 모아두기 == checker == 1
        ...
        for (int i = 0; i < size; i++) {
            WeeklyQuest now = can_recive[i] as WeeklyQuest;
            ...
            //prefab.AddComponent<WeeklyQuestRecive>();
            prefab.transform.Find("Recive").gameObject
                            .GetComponent<Button>().onClick
                            .AddListener(() => Recive(now.Id));
            //prefab.GetComponent<WeeklyQuestRecive>().Id = now.Id;
        ...
        size = normal.Count;
        for (int i = 0; i < size; i++) {
            WeeklyQuest now = normal[i] as WeeklyQuest;
            GameObject prefab = Instantiate(_quest_obj, Content.transform.position, Content.transform.rotation);
            // value 설정
            prefab.transform.SetParent(Content.transform, false);
            prefab.transform.Find("BackGround").gameObject.GetComponent<Image>().color = DEFAULT_BACKGROUND;
        ...
        for (int i = 0; i < size; i++) {
            WeeklyQuest now = already_do[i] as WeeklyQuest;
            GameObject prefab = Instantiate(_quest_obj, Content.transform.position, Content.transform.rotation);
            // value 설정
            prefab.transform.SetParent(Content.transform, false);
    ...
    public override void AllRecive() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (achieve.CanRecive) {
                ...
                // 수정된 checker값 업데이트
                query = builder.Append("UPDATE weeklyquest SET checker=")
    ...
    public override void Recive(int id) {
        WeeklyQuest quest = datahub.WeeklyQuest[id] as WeeklyQuest;
        string announce;
