...
public class Unit
{
    ...
    private int max_upgrade_value = 15;     // 최대 강화 수치
    private string type;                    // 형태 타입
    private string grade;                   // 등급
    private int need_gold;                  // 강화시 필요 골드량 -> 현재 강화 수에 비례하여 증가 (0 == 기본, 1 => 2배~~)
    private int piece;                      // 현재 보유 조각 갯수
    private int need_piece;                 // 강화시 필요 조각 갯수를 계산할 변수
    ...
    public string Show_Info() {
        return (id + ":" + name + "/" + nickname + "/" + attack + "/" + attack_speed + "/" + upgrade_figures + "/" + upgrade_value + "/" + type + "/" + grade);
    }
}
