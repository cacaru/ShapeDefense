...
도형 모티프의 유닛으로 적을 막아내는 모바일 타워 디펜스 게임 (Unity 6).

- **회사명**: SunhosWorld
- **제품명**: ShapeDefense
- **플랫폼**: Android (모바일 세로 모드, 기본 해상도 640x960)
- **엔진**: Unity 6000.0.63f1
...
- **유닛 시스템** — 도형/타입/등급별 유닛, 강화(Upgrade), 합성(Combine), 판매(Sell), 리롤(Reroll)
- **전투 시스템** — 자동 공격, 공격 범위(AttackArea), 총알 풀링, 보스 처치 보상
- **8개 맵 / 8가지 적 이동 패턴** — `MapScene1` ~ `MapScene8`, `EnemyMove1` ~ `EnemyMove8`
- **콘텐츠 시스템**
  - 업적(Achievement)
...
- **수익화** — Google Mobile Ads 연동 (`GoogleAdvManager`)
- **데이터 저장** — SQLite 기반 로컬 DB (`Assets/StreamingAssets/ShapeDefenseDB.db`)
...
│   ├── Resources/
│   │   ├── Prefabs/                # Bullet, Enemy, PieceUnit, Quest 등 런타임 로드용 프리팹
│   │   └── Sprite/                 # Background, Bullet, Enemy, Unit, Icon, Stage, Tile (스프라이트 아틀라스 포함)
│   ├── Scenes/
